...
export const EditVoucher = async (req, res) => {
    try {
        ...
        if(req.body.amount > 0 || req.body.percent > 0 ) { 
            const voucher = await Discount.findByIdAndUpdate({_id:req.params.id}, req.body ,  {returnDocument: "after"}).exec()
           return res.json({
                ...voucher,
                code:200
           })
...
export const checkVoucher = async (req,res) => {    
    try {
        ...
        const findOrder = await Order.find({voucher: findVoucher._id, userID: findUser._id}).exec()
        ...
        if(req.body.update == true) {
            ...
            await Discount.findByIdAndUpdate({_id: findVoucher._id}, findVoucher, {returnDocument: "after"}).exec()
        }
...
export const removeDiscount = async (req,res) => {
    try {
        const response = await Discount.findByIdAndDelete({_id: req.params.id}).exec()
        if(response == null) {
            return res.json({
                result: 500,
